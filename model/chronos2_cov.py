"""Chronos-2 conditioned on covariates (H1) — improve the champion's keeper expert.

The plain chronos-2 leg runs zero-shot on the raw sales context only (chronos2.py).
Chronos-2 v2.x supports covariates via predict_df(df, future_df, ...): columns in df that
are NOT the target are past covariates; columns also present in future_df become KNOWN
future covariates. onpromotion is known for the full horizon (in train for VAL, test.csv
for TEST) -> ideal known-future covariate. Oil is past-only (future unknown). Holiday flag
is known-future.

MUST run in .venv_chronos2/bin/python (py3.11 + chronos-forecasting 2.2.2).

Saves:
  submissions/submission_chronos2_cov{suffix}.csv

The leg's standalone RMSLE (sigma_chronos2-cov) is obtained by submitting that CSV
to Kaggle (notebook §4.2), not by an extra holdout pass.
"""
...
def build_holiday_flag():
    """National-level special-day flag per date (simple, locale-agnostic).
    1 if an active (non-transferred-away) Holiday/Additional/Bridge/Event/Transfer day."""
    hol = pd.read_csv(DATA / "holidays_events.csv", parse_dates=["date"])
    active = hol[(hol["transferred"] == False) & (hol["type"] != "Work Day")]
...
def make_panel(train, test, use_oil, use_holiday):
    """Long panel of all series with continuous daily timestamps, target + covariates."""
...
def run_pass(pipe, panel, hol_flag, oil_daily, anchor, batch_size, use_oil, use_holiday):
    """Build df (context) + future_df (horizon) and call predict_df."""
